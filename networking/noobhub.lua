...
noobhub = {
    ...
    new = function (params) -- constructor method
        ...
        if (not params.server  or not  params.port) then
            netHud:status("Noobhub requires server and port to be specified");
            return false;
        end;
        ...
        function self:subscribe(params)
            ...
            netHud:status("Calling game server "..self.server..":"..self.port)

            self.sock, error_message = socket.connect(self.server,  self.port)

            if (self.sock == nil) then
                netHud:status("Server connection error: "..error_message)
                return false;
            end
            ...
            netHud:status("Connected to game server")
        ...
        function self:reconnect()
            ...
            netHud:status("Connection lost: attempt to reconnect...");
        ...
        function self:publish(message)
            -- TODO: add retries
            if (self.sock == nil) then
                netHud:status("Attempt to publish without valid subscription (bad socket)")
                self:reconnect()
                return false;
            end
            ...
            if (send_result == nil) then
                netHud:status("Server publish error: "..message..'  sent '..num_bytes..' bytes');
